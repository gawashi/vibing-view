...
describe('quoteSymbols', () => {
  it('collects visible cell symbols, skipping empty cells', () => {
    expect(quoteSymbols([cell('a', 'AAPL'), cell('b', null)], { rows: 1, cols: 2 })).toEqual(['AAPL'])
  })
  it('unions watchlist symbols and de-dupes', () => {
    const out = quoteSymbols([cell('a', 'AAPL')], { rows: 1, cols: 1 }, ['AAPL', 'MSFT'])
  ...
  it('ignores cells beyond the visible count for the shape', () => {
    ...
    expect(quoteSymbols([cell('a', 'AAPL'), cell('b', 'MSFT')], { rows: 1, cols: 1 })).toEqual(['AAPL'])
  })
})
